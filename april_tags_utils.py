...
'''
rotates roll then pitch then yaw
'''
def rotate(vector, yaw, pitch, roll):
    vec = copy.deepcopy(vector)
    vec = [(vec[0] * math.cos(roll)) - (vec[1] * math.sin(roll)),
            (vec[0] * math.sin(roll)) + (vec[1] * math.cos(roll)), 
            vec[2]]
    vec = [vec[0] * math.cos(pitch) + vec[2] * math.sin(pitch),
            vec[1], 
            vec[2] * math.cos(pitch) - vec[0] * math.sin(pitch)]
    vec = [vec[0],
            vec[1] * math.cos(yaw) - vec[2] * math.sin(yaw), 
            vec[1] * math.sin(yaw) + vec[2] * math.cos(yaw)]
    return np.array(vec)
'''
return 3d vector representing tags yaw, pitch and roll, and the shape after rotation assumes tag shape
tag_shape: an array of the coordinates of the tags corners (in global coordinates)
tag_corners: an array of the coordinate of the tags corners (in camera coordinates)
center: vector of tag center (in camera coordinates)
'''
def get_tag_rotation(tag_corners, center):
    delta_corners = [v - center for v in tag_corners]
    # yaw
    delta_y = delta_corners[3][1] - delta_corners[0][1]
    delta_z = delta_corners[3][2] - delta_corners[0][2]
    yaw = math.atan2(delta_z, delta_y)
    delta_corners = [[vec[0],
                      vec[1] * math.cos(-yaw) - vec[2] * math.sin(-yaw), 
                      vec[1] * math.sin(-yaw) + vec[2] * math.cos(-yaw)] for vec in delta_corners]
    # pitch
    delta_z = delta_corners[1][2] - delta_corners[0][2]
    delta_x = delta_corners[1][0] - delta_corners[0][0]
    pitch = math.atan2(delta_x, delta_z) - 0.5*math.pi
    delta_corners = [[vec[0] * math.cos(-pitch) + vec[2] * math.sin(-pitch),
                      vec[1], 
                      vec[2] * math.cos(-pitch) - vec[0] * math.sin(-pitch)] for vec in delta_corners]
    
    # roll
    delta_y = delta_corners[1][1] - delta_corners[0][1]
    delta_x = delta_corners[1][0] - delta_corners[0][0]
    roll = math.atan2(delta_y, delta_x) 
    delta_corners = [[(vec[0] * math.cos(-roll)) - (vec[1] * math.sin(-roll)),
                      (vec[0] * math.sin(-roll)) + (vec[1] * math.cos(-roll)), 
                      vec[2]] for vec in delta_corners] 
    
    
    
    return [pitch, yaw, roll], [np.array(vec) + center for vec in delta_corners]
...
def draw_tags(frame, all_corners_3d, F_LENGTH, axis_line_length=10):
    if (frame is not None):
        for corners_3d in all_corners_3d:
            ...
            # proj_center =  project_point(center_3d[0], center_3d[1], center_3d[2], width, height, F_LENGTH)
            proj_cros_axis_point = get_cross_axis_no_flip(corners_3d[0], corners_3d[1], center_3d, axis_line_length)
